...
class LterExt:
    ...
    def service_records(self):
        service_config = config.LTER_RECORD_SERVICE_CONFIG
    ...
    def resource_records(self):
        return config.LTER_RECORD_RESOURCE_CLASS(
    ...
    def published_service_records(self):
        from lter.services.records.published.config import LterPublishedServiceConfig
        from lter.services.records.published.service import LterPublishedService

        return LterPublishedService(
    ...
    def service_files(self):
        service_config = config.LTER_FILES_SERVICE_CONFIG
        if hasattr(service_config, "build"):
            config_class = service_config.build(self.app)
        else:
            config_class = service_config()
    ...
    def service_draft_files(self):
        service_config = config.LTER_DRAFT_FILES_SERVICE_CONFIG
        if hasattr(service_config, "build"):
            config_class = service_config.build(self.app)
        else:
            config_class = service_config()
