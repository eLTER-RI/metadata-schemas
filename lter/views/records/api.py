...
def create_api_blueprint(app):
    """Create LterRecord blueprint."""
    blueprint = app.extensions["lter"].resource_records.as_blueprint()
    ...
    funcs = globals()
    funcs = [
        v for k, v in funcs.items() if k.startswith("init_addons_lter") and callable(v)
    ]
    for func in funcs:
...
def init_addons_lter_requests(state):
    app = state.app
    requests = app.extensions["invenio-requests"]

    from lter import config

    for er in getattr(config, "LTER_ENTITY_RESOLVERS", []):
        requests.entity_resolvers_registry.register_type(er)


def init_addons_lter_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["lter"]
